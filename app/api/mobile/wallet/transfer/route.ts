...
import { TransferSchema, zodValidate } from '@/lib/validators';

export async function POST(request: Request) {
  try {
    const body = await request.json();

    console.log('💱 [API] POST /api/mobile/wallet/transfer');
    ...
    const { senderId: userId, receiverId: toUserId, amount, note: description, pin } = validation.data;

    console.log('💱 [API] Validated transfer:', { userId, toUserId, amount });

    if (userId === toUserId) {
      return NextResponse.json(
        { success: false, error: 'Vous ne pouvez pas transférer à vous-même' },
        { status: 400 }
      );
    }
